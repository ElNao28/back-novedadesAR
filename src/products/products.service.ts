...
export class ProductsService {
  constructor(
    ...
    private imagenesRepository: Repository<Imagenes>,
  ...
  async create(
    ...
    file: { imagen?: Express.Multer.File[] },
  ) {
    try {
  ...
  async findOne(id: number) {
    ...
    for (let i = 0; i < product.comentarios.length; i++) {
      comentarios.push({
        ...
        usuario:
          product.comentarios[i].usuario.name +
          ' ' +
          product.comentarios[i].usuario.lastname +
          ' ' +
          product.comentarios[i].usuario.motherLastname,
      });
    }
    return {
      id: product.id,
  ...
  async formPago(res: ResDto[]) {
    ...
    const client = new MercadoPagoConfig({
      accessToken:
        'TEST-3954097920512827-030816-523113fab0eca51c8a57d53e2cf509d6-1719432312',
    });
    ...
    await preference
      .create({
        body: {
          ...
          notification_url:
            'https://4c48-201-162-250-184.ngrok-free.app/products/res-pago/' +
            res[0].idUser +
            '/card/' +
            res[0].idCard,
        },
    ...
    if (data) {
      ...
      if (pago.status === 'approved') {
        this.ventasService.addVenta(
          parseInt(idUser),
          pago.additional_info.items,
          idCard,
          pago.transaction_details.total_paid_amount,
          pago.date_approved,
        );
      }
    }
  ...
  async getProductByTypeUser(typeUser: number) {
    try {
      ...
      const tipoCliente = await this.httpService
        .post<{ tipo_cliente: number }>(
          'https://server-m3.onrender.com/predict',
          payload,
        )
        .pipe(
      ...
      switch (tipoCliente) {
        ...
        case 1:
          console.log('tipo 1');
          let gender;
          if (foundUser.gender === 'F') gender = 'M';
          else gender = 'H';
          productos = await this.producRepository.find({
            where: {
